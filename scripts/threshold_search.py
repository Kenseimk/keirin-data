# -*- coding: utf-8 -*-
"""
新モデル(コンボ) combo_score閾値サーチ
訓練: ~2025-12  /  テスト: 2026-01〜04
combo_scoreのパーセンタイル閾値ごとの的中率・回収率を比較
"""
...
print(f"テスト期間: {TEST_MONTH_START} 〜 {TEST_MONTH_END}")
...
for race_id, g in combo_te.groupby("race_id"):
    ...
    parts = re.split(r"[-=]", str(san_combo)) if pd.notna(san_combo) else []
    is_hit = False
    if len(parts) == 3:
        try: is_hit = (int(parts[0])==int(best["b1"]) and int(parts[1])==int(best["b2"]) and int(parts[2])==int(best["b3"]))
        except: pass
    pred_rows.append({
        "race_id": race_id,
        "combo_score": float(best["combo_score"]),
        "hit": is_hit,
        "payout": best["san_payout"] if is_hit else 0,
        "month": str(race_id)[:7] if isinstance(race_id, str) else None,
    })

pred_df = pd.DataFrame(pred_rows)
# race_idからdateを引く
race_date = df_test.groupby("race_id")["date"].first()
pred_df["date"] = pred_df["race_id"].map(race_date)
pred_df["month"] = pred_df["date"].str[:7]

total = len(pred_df)
print(f"\n全予測レース数: {total}  スコア範囲: {pred_df['combo_score'].min():.4f} 〜 {pred_df['combo_score'].max():.4f}")

# ── パーセンタイル閾値サーチ ──
print("\n" + "="*75)
print("  combo_score 閾値サーチ（スコア上位X%のレースだけ買う）")
print("="*75)
print(f"{'上位%':>7}  {'閾値':>8}  {'買い数':>7}  {'的中':>5}  {'的中率':>7}  {'払戻合計':>10}  {'回収率':>7}")
print("-"*75)
...
for pct in percentiles:
    threshold = np.percentile(pred_df["combo_score"], 100 - pct)
    sub = pred_df[pred_df["combo_score"] >= threshold]
    n = len(sub); h = sub["hit"].sum(); pay = sub["payout"].sum()
    hr = h/n*100 if n else 0
    roi = pay/(n*100)*100 if n else 0
    marker = " ◀" if hr > best_hit_rate else ""
    if hr > best_hit_rate:
        best_hit_rate = hr
        best_pct = pct
    print(f"  上位{pct:3d}%  {threshold:.4f}  {n:>7}  {h:>5}  {hr:>6.1f}%  {int(pay):>10,}円  {roi:>6.1f}%{marker}")

# ── 月別×閾値クロス集計 ──
print("\n" + "="*75)
print("  月別 × 閾値クロス（的中率）")
print("="*75)
months = sorted(pred_df["month"].dropna().unique())
pcts_cross = [20, 30, 40, 50, 70, 100]
header = f"{'月':>9}" + "".join(f"  上位{p:3d}%" for p in pcts_cross)
print(header)
print("-"*75)

for month in months:
    row_str = f"  {month}"
    for pct in pcts_cross:
        threshold = np.percentile(pred_df["combo_score"], 100 - pct)
        sub = pred_df[(pred_df["month"]==month) & (pred_df["combo_score"]>=threshold)]
        n = len(sub); h = sub["hit"].sum()
        hr = h/n*100 if n else 0
        row_str += f"  {hr:5.1f}%({h}/{n})"
    print(row_str)
...
# ── combo_score分布確認 ──
print("\n" + "="*50)
print("  combo_score 分布（的中 vs 外れ）")
print("="*50)
hit_scores   = pred_df[pred_df["hit"]==True]["combo_score"]
miss_scores  = pred_df[pred_df["hit"]==False]["combo_score"]
print(f"  的中レース  平均score: {hit_scores.mean():.4f}  中央値: {hit_scores.median():.4f}  件数: {len(hit_scores)}")
print(f"  外れレース  平均score: {miss_scores.mean():.4f}  中央値: {miss_scores.median():.4f}  件数: {len(miss_scores)}")

bins = [0, 0.001, 0.002, 0.003, 0.005, 0.01, 0.02, 0.05, 1.0]
print(f"\n  {'スコア区間':>20}  {'件数':>6}  {'的中':>5}  {'的中率':>7}  {'回収率':>7}")
for lo, hi in zip(bins[:-1], bins[1:]):
    sub = pred_df[(pred_df["combo_score"]>=lo) & (pred_df["combo_score"]<hi)]
    n = len(sub); h = sub["hit"].sum(); pay = sub["payout"].sum()
    hr = h/n*100 if n else 0
    roi = pay/(n*100)*100 if n else 0
    print(f"  [{lo:.4f}, {hi:.4f})  {n:>6}  {h:>5}  {hr:>6.1f}%  {roi:>6.1f}%")

print(f"\n→ 的中率が最も高い閾値: 上位{best_pct}% (的中率 {best_hit_rate:.1f}%)")
