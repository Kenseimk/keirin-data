...
# 出力（月別）
for month in [TEST_MONTH_START, "2026-02", TEST_MONTH_END]:
    df_m = df_out[df_out["date"].str[:7] == month]
...
print("  月別サマリー比較")
print("="*70)
print(f"{'月':8} {'':12} {'旧レース':>8} {'旧件数':>6} {'旧的中':>6} {'旧回収率':>8} | {'新レース':>8} {'新件数':>6} {'新的中':>6} {'新回収率':>8}")
print("-"*70)
...
for month in [TEST_MONTH_START, "2026-02", TEST_MONTH_END]:
    ...
    o = df_m[df_m["in_old"]]
    n = df_m[df_m["in_new"]]
    ob = o["old_bets"].sum(); oh = o["old_hit"].sum(); op = o["old_payout"].sum()
    nb = len(n); nh = n["new_hit"].sum(); np_ = n["new_payout"].sum()
    or_ = op/max(ob*100,1)*100; nr_ = np_/max(nb*100,1)*100
    print(f"{month}   旧: {len(o):>6}R {ob:>6}件 {oh:>6}的中 {or_:>7.1f}% | 新: {len(n):>6}R {nb:>6}件 {nh:>6}的中 {nr_:>7.1f}%")
