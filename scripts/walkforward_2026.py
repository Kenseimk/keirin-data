...
test_months = [m for m in sorted(df_model["date"].str[:7].unique()) if m >= "2026-01"]

all_rows = []
for test_month in test_months:
    print(f"  {test_month} 処理中...", flush=True)
...
print("\n=== 2026年 現在フィルター (top_score>=95, score_gap>=2, 7車) ===")
print(f"{'月':<10} {'賭件数':>6} {'的中':>5} {'的中率':>7} {'投資':>8} {'払戻':>10} {'回収率':>7}  的中払戻")
print("-" * 75)
for month, g in rp.groupby("month"):
    bets = len(g); hits = int(g["hit"].sum()); invest = bets * 100
    payout = int(g[g["hit"]]["san_payout"].sum()) if hits > 0 else 0
    roi = payout / invest * 100 if invest > 0 else 0
    pays = sorted(g[g["hit"]]["san_payout"].tolist(), reverse=True)
    pay_str = "  ".join([f"{int(p):,}円" for p in pays]) if pays else "-"
    print(f"{month:<10} {bets:>6} {hits:>5} {hits/bets*100:>6.1f}% {invest:>8,} {payout:>10,} {roi:>6.0f}%  {pay_str}")

total_bets = len(rp); total_hits = int(rp["hit"].sum())
total_invest = total_bets * 100
total_payout = int(rp[rp["hit"]]["san_payout"].sum()) if total_hits > 0 else 0
print("-" * 75)
print(f"{'合計':<10} {total_bets:>6} {total_hits:>5} {total_hits/total_bets*100:>6.1f}% "
      f"{total_invest:>8,} {total_payout:>10,} {total_payout/total_invest*100:>6.0f}%")
