# -*- coding: utf-8 -*-
"""フィルターなし vs 現行フィルター ウォークフォワード比較（旧モデル・動的フォーメーション）"""
...
# 各フィルターの月別結果を記録
records = {k: [] for k in ["no_filter", "ts95_only", "current"]}
...
for test_month in test_months:
    ...
    def add_record(key, sub):
        ...
        records[key].append({
            ...
            "hit_rate": hits/len(sub)*100, "roi": pay/(len(sub)*100)*100
        })

    add_record("no_filter", rp)
    add_record("ts95_only", rp[rp["top_score"] >= 95])
    ...
    print(f"  {test_month}  全:{len(rp):4d}件/{h_nf}的中  現行:{len(rp[(rp['top_score']>=95)&(rp['score_gap']>=2)&(rp['n_players']==7)]):3d}件/{h_cur}的中")

print("\n=== 結果サマリー ===")
labels = {"no_filter": "フィルターなし   ", "ts95_only": "top_score>=95のみ ", "current": "現行(+gap+7車)    "}
for key, label in labels.items():
    df = pd.DataFrame(records[key])
    if df.empty: continue
    print(f"[{label}]  月平均 的中率:{df['hit_rate'].mean():.1f}%  回収率:{df['roi'].mean():.1f}%"
          f"  | 総賭:{df['bets'].sum():,}  総的中:{df['hits'].sum()}")

print("\n--- 月別ROI比較 ---")
df_nf  = pd.DataFrame(records["no_filter"]).set_index("month")["roi"].rename("フィルターなし")
df_ts  = pd.DataFrame(records["ts95_only"]).set_index("month")["roi"].rename("ts95のみ")
df_cur = pd.DataFrame(records["current"]).set_index("month")["roi"].rename("現行")
comp = pd.concat([df_nf, df_ts, df_cur], axis=1)
